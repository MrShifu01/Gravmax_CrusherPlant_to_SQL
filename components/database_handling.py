...
def check_and_create_table(cursor, table_name, df):
    try:
        cols = ", ".join([f"[{col}] NVARCHAR(MAX)" if col !=
                         'PrimaryKey' else "[PrimaryKey] INT" for col in df.columns])
        cursor.execute(
            f"""IF OBJECT_ID('{table_name}', 'U') IS NULL CREATE TABLE {table_name} ([PrimaryKey] INT IDENTITY(1,1) PRIMARY KEY, {cols})""")
...
def upsert_data_to_sql(conn, cursor, table_name, df, file_path, db_name):
    try:
        ...
        on_conditions = " AND ".join(
            [f"Target.{col} = Source.{col}" for col in columns_bracketed])

        for index, row in df.iterrows():
            # Get the values excluding 'PrimaryKey' column
            values = ", ".join([f"'{value}'" for col, value in zip(
                df.columns, row.astype(str).values) if col != 'PrimaryKey'])
